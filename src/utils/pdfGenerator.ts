...
export async function generatePDF(songs: { title: string; artist: string; year: number; spotifyUri: string }[]) {
  const pdf = new jsPDF({
  ...
  for (let i = 0; i < songs.length; i++) {
    ...
    if (i % (TILES_PER_ROW * 4) === 0) {
      ...
      pdf.text('generate.blindsongscanner.com', pageWidth / 2, pageHeight - 0.5, { align: 'center' });
      pdf.setTextColor(0);
    }
    ...
    pdf.setFontSize(12);
    pdf.text(song.artist, x + 3, y + 1, { maxWidth: TILE_SIZE_CM - 0.5, align: 'center' });
    pdf.setFontSize(14);
    pdf.text(song.title, x + 3, y + 3, { maxWidth: TILE_SIZE_CM - 0.5, align: 'center' });
    pdf.setFontSize(18);
    pdf.text(String(song.year || ''), x + 3, y + 5.3, { maxWidth: TILE_SIZE_CM - 0.5, align: 'center' });
    ...
    if (i % (TILES_PER_ROW * 4) === (TILES_PER_ROW * 4) - 1) {
      ...
      for (let j = 0; j < TILES_PER_ROW * 4; j++) {
        ...
        try {
          const spotifyUrl = songs[songIndex].spotifyUri.replace('spotify:track:', 'https://open.spotify.com/track/');
