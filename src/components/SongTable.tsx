...
interface TextInputWithWarningProps {
  value: string;
  onChange: (value: string) => void;
  isLong: boolean;
}

const TextInputWithWarning: React.FC<TextInputWithWarningProps> = ({ value, onChange, isLong }) => (
    ...
    <div className="overflow-x-auto -mx-3 sm:mx-0">
      <table className="min-w-full bg-white shadow-md rounded-lg table-fixed">
        <thead className="bg-gray-100">
          <tr>
            <th className="w-5/12 px-2 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Title</th>
            <th className="w-5/12 px-2 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Artist</th>
            <th className="w-2/12 px-2 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Year</th>
          </tr>
        </thead>
              ...
              <td className="px-2 sm:px-6 py-2 sm:py-4">
                <div className="flex items-center">
                  <input
                    type="number"
                    value={song.year}
                    onChange={(e) => onEdit(index, 'year', parseInt(e.target.value, 10))}
                    className="block w-16 sm:w-20 px-2 py-1 text-xs sm:text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                  />
                  <Pencil className="w-4 h-4 ml-2 text-gray-400" />
                </div>
              </td>
